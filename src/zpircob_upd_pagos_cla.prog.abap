...
*   Rescata cabecera de documentos de pago
    SELECT FROM bkpf
      FIELDS bukrs, belnr, gjahr, monat, blart, cpudt, cputm, waers
...
*   Si no hay datos, actualiza tabla de control con los parámetros actuales
    IF gt_bkpf[] IS INITIAL.
      me->upd_control( EXPORTING iv_bukrs     = iv_bukrs
                                 iv_feuej_cob = iv_fehas
                                 iv_houej_cob = iv_hohas
                     ).
      RETURN.
    ENDIF.
  ...
  METHOD upd_logtab.
    ...
    IF gt_data[] IS INITIAL.
      me->upd_control( EXPORTING iv_bukrs     = p_bukrs
                                 iv_feuej_cob = p_fehas
                                 iv_houej_cob = p_hohas
                     ).
...
***
    IF sy-subrc = 0.
      me->upd_control( EXPORTING iv_bukrs     = lt_zpircob_sum_c[ 1 ]-bukrs
                                 iv_feuej_cob = gv_max_date
                                 iv_houej_cob = gv_max_time
                     ).
...
*-> it_pendi TYPE gtt_pendi
    ...
    DELETE FROM zpircob_pend WHERE stblg = ' '.
