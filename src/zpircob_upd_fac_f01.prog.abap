...
FORM val_fecha CHANGING cv_felog TYPE zde_fefin_uef
  ...
  SELECT SINGLE FROM zpircob_control FIELDS fefin_uef, hofin_uef
    WHERE bukrs = @p_bukrs
    INTO @DATA(ls_control).

  IF sy-subrc = 0 AND NOT ls_control-fefin_uef IS INITIAL.
    lv_year  = ls_control-fefin_uef+0(4).
    lv_year  = lv_year + 1.

    lv_femax = |{ lv_year }{ ls_control-fefin_uef+4(4) } |.
    cv_felog = ls_control-fefin_uef.
    cv_holog = ls_control-hofin_uef.
  ELSE.
